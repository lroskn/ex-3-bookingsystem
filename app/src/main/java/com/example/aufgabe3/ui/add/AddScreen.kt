            ...
            Button(
                onClick = {
                    ...
                    if (arrivalDate == null || departureDate == null) {
                        println("Please select a date range")
                        return@Button
                    }

                    if(departureDate!!.isBefore(arrivalDate)){
                        println("Departure date must be after arrival date")
                        return@Button
                    }
                    ...
                    println("Booking entry saved: $bookingEntry")
    ...
    if (showDateRangePicker) {
        DateRangePickerModal(
            onDateRangeSelected = { dateRange ->
                dateRange.first?.let { arrivalMillis ->
                    arrivalDate = LocalDate.ofEpochDay(arrivalMillis / (24 * 60 * 60 * 1000))
                }
                dateRange.second?.let { departureMillis ->
                    departureDate = LocalDate.ofEpochDay(departureMillis / (24 * 60 * 60 * 1000))
                }
